...
resource_path: ["../"]
abstract: >
  The thermodynamic effect on a biochemical reaction depends principally on the
  standard-condition Gibbs energy changes of the compounds that the reaction
  consumes and produces. The state of the art method for estimating these
  parameters involves a statistical model called group contribution. We highlight
  a problem with the group contribution model that limits its usefulness in
  downstream modelling applications: the group contribution model typically
  produces a rank-deficient uncertainty covariance matrix. We present a new model
  that avoids this issue, generates more accurate estimates than the current
  state of the art and has unlimited reaction coverage. Our model includes
  parameters for both chemical groups and compounds, using a hierarchical
  component to balance information pertaining to these two levels, and addresses
  the issue of stoichiometric collinearity using QR decomposition. We provide a
  Gaussian approximation to our model's posterior distribution that can be used
  for downstream modelling. Our results can be replicated straightforwardly by
...
The rate of an enzyme-catalysed reaction is closely related to the reaction's
thermodynamic properties, particularly the standard condition Gibbs free energy
of formation, or "formation energy" of each consumed and produced compound.
Under standard conditions the overall rate $v$ of a reaction involving
$n\in\mathbb{N}$ species depends on formation energies $\Delta_f
G\in\mathbb{R}^n$, as well as stoichiometries $\vec{s}\in\mathbb{R}^n$,
concentrations $c\in\mathbb{R}_+^n$, temperature $T\in \mathbb{R}_+$, gas
constant $R\approx0.008314$ and non-thermodynamic effects $\psi \in
\mathbb{R}_+$ according to equation \eqref{eq-thermodynamic-effect}:
...
directly measured, in order to estimate formation energies it is necessary to
rely on measurements of $\Delta_rG$. @beberEQuilibratorPlatformEstimation2021
describes the software package and website eQuilibrator, which provides access
to a large database of such measurements derived from
@goldbergThermodynamicsEnzymecatalyzedReactions2004 among other sources.
...
measurement indexes to columns of $S$, so that $reaction(i)$ is the index of
...
groups, 'group contribution' model. In equation \eqref{eq-group-additivity},
$G\in\mathbb{R}^{m\times g}$ is a group incidence matrix that maps combinations
of $m$ compounds and $g$ groups to stoichiometric coefficients and $\Delta_gG$
...
Using this model's estimates $\hat{\Delta_gG}$ for the parameters $\Delta _gG$,
...
energy change $\hat{\Delta_rG}_j$ zero according to the group contribution
...
modelling problem. If $\Delta_gG$ is estimated by maximum likelihood
estimation, the covariance matrix of the estimator vector
$\hat{\Delta_fG}=G\hat{\Delta_gG}$ is given by equation
...
$Cov(\hat{\Delta_fG})$ is guaranteed to be rank-deficient. If
$Cov(\hat{\Delta_fG})$ is used as the covariance matrix of a multivariate
...
regions of $\Delta_fG$ space. If the true parameter allocation lies in such a
...
groups, it is not possible to say in general which reactions are affected by
...
This improved group contribution model leads has the same maximum likelhood
estimator $\hat{\Delta_fG}$ as the current group contribution method. However
in the improved model the covariance matrix $Cov(\hat{\Delta_fG})$ is found by
...
In this model $Cov(\hat{\Delta_fG})$ is guaranteed to be full rank, as
...
The prior distribution for the group formation energy vector $\Delta_gG$ is
...
vector $\Delta_gG$ by the group decomposition matrix and then the
...
In this parameterisation the formation energy parameters $\Delta_fG$ and
$\Delta_gG$ can be recovered as shown in equation \eqref{eq-qr-recovery}, in
...
$\eta_{new}\sim N(0, \tau)$. Now recover $\Delta_fG_{new}$ as in
\eqref{eq-qr-recovery}.
...
distribution for $\vec{q}$. Next find $\Delta_gG_{new}$ using equation
\eqref{eq-qr-recovery}, then generate $\eta_{new}$ and recover
$\Delta_fG_{new}$ using the same method as for case 2.
...
$\Delta_fG$ parameter samples. We have done this for our results and provide
...
contribution $\Delta_rG$ as well as the residual for our model's marginal
...
![Comparison of residuals](plots/posterior_predictive_comparison.png){#fig-residuals-full-dataset}
