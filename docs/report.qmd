...
As it is a state function, the Gibbs energy of a reaction $\dgr$ is solely a function of the Gibb's energy of formation $\dgr$ of its substrates.
...
More formally, given a set of $n$ measurements $y\in\mathbb{R}^n$ of the standard reaction energies $\dgr G^\degree$ of $r$ reactions' standard condition Gibbs energy changes and a stoichiometric matrix $S\in\mathbb{R}^{m\times r}$ providing the stoichiometric coefficients of $m$ compounds in each measured reaction and a vector of errors $\epsilon_m \in \mathbb{R}^{n}$, formation energies can be estimated using the generative model in equation \eqref{eq-rc}.

\begin{align}
&y = S^\top \dgf + \epsilon_m \label{eq-rc} \\
&\epsilon_m \sim N(\hat{y}_i, \sigma) \nonumber
\end{align}

where N($\mu$, $\sigma$) follows the Stan notation, with mean $\mu$ and standard deviation $\sigma$.
This also allows one to estimate $\dgr$ for unmeasured reactions, as long as they only involve metabolites present in the model. 
...
As with $\dgr$ and $\dgf$, and because group matrix symbol $\mathbf{G}$ and the 'G' of the standard group energies $\dgg$ can be easily confused, we refer herein to the standard group energies with $\dgg$. 
...
<!--Make note that this is the LR proposed in the original model-->
\begin{align}
...
Secondly, \epsilon represents both the measurement error $\epsilon_m$ of \eqref{eq-reactant-contribution} but also error that results from all other factors that affect formation energies that are not represented in this simple linear model over groups, which we refer to herein as model error $\epsilon_o$.
...
Additionally, estimates of variance are also important in a number of applications.
For example, thermodynamic metabolic flux analysis originally required $\dgr$ marginal flux estimates, which was recently improved upon by <!--citation--> by making use of full covariance estimates of the same.
In @beberEQuilibratorPlatformEstimation2021, the method for calculating full covariance estimates for both $\dgr$ and $\dgf$ is clearly described. 
<!--Add covaraince estimation equations here-->
...
This estimate of the Gibbs energy of reaction $\dgr$ has three sources of error: the error in the estimate $\hat{\dgg}$, which is normally distributed, the measurement error (usually assumed to be normally distributed) and finally the error that arises from an imperfect model.
That is: 
...
The problem stems from the fact that model mispecification errors are inherently a result of problems in the group contribution model and not the relationship between $\dgr$ and $\dgf$ (the known physical law), yet the model is fit with additive errors on $\dgr$.
This has a number of consequences.
Firstly, it leads to violations in the state function \eqref{eq-dgr-dgf-law}.
For example, if we assume known measurements and parameters for the reactions A and B, both of which involve metabolites C and D, we get the following equation: 

\begin{align} 
\dgr = S^\top \mathbf{G} \dgg + \epsilon_m 
\end{align}

with realised values for $\epsilon_m$ of x and y.
However, C and D are not random variables giving $C-D\neq A$ and $C-D\neq B$.

Another consequence is the reduced degrees of freedom of $\dgf$.
In order to be useful, the group matrix $\mathbf{G}$ necessarily has fewer groups than metabolites, giving $\dgf$ degrees of freedom equivalent to Rank($\mathbf{G}$), which does not allow support for the full range of formation energies.
...
<!--It should be stated that this is a manifestation of the rank problem above. Maybe shift this section up?-->
One way in which the reduced degrees of freedom manifests is related to compounds that have the same group composition.
Suppose two compounds $a$ and $b$ have the same group composition vector $\vec{g}_{ab}$.
...
<!--Can there be an example here from reactions that are *only* from RC metabolites?-->
